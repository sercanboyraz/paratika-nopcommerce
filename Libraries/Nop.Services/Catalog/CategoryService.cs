...
namespace Nop.Services.Catalog
{
    ...
    public partial class CategoryService : ICategoryService
    {
        ...
        {
            return _categoryRepository.Table.Count(x => x.Name == categoryName) > 0;
        }

        public int LastDisplayOrder()
        {
            return _categoryRepository.Table.OrderBy(x => x.DisplayOrder).LastOrDefault().DisplayOrder;
        }

        public Category GetCategoryName(string categoryName)
        {
            return _categoryRepository.Table.FirstOrDefault(x => x.Name == categoryName);
        }
    }
}