CREATE FUNCTION [sercan].[nop_splitstring_to_table]
(
    @string NVARCHAR(MAX),
    @delimiter CHAR(1)
)
...
CREATE FUNCTION [sercan].[nop_getnotnullnotempty]
(
...
CREATE FUNCTION [sercan].[nop_getprimarykey_indexname]
(
...
CREATE FUNCTION [sercan].[nop_padright]
(
    @source INT, 
    @symbol NVARCHAR(MAX), 
    @length INT
...
CREATE PROCEDURE [sercan].[ProductLoadAllPaged]
(
...
CREATE PROCEDURE [sercan].[ProductTagCountLoadAll]
(
	@StoreId int
)
AS
...
CREATE PROCEDURE [sercan].[FullText_IsSupported]
AS
BEGIN	
	EXEC('
...
CREATE PROCEDURE [sercan].[FullText_Enable]
AS
BEGIN
	--create catalog
	EXEC('
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[Product]''))
		...
		KEY INDEX [' + sercan.[nop_getprimarykey_indexname] ('Product') +  '] ON [nopCommerceFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[LocalizedProperty]''))
		...
		KEY INDEX [' + sercan.[nop_getprimarykey_indexname] ('LocalizedProperty') +  '] ON [nopCommerceFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	EXEC(@create_index_text)
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[ProductTag]''))
		...
		KEY INDEX [' + sercan.[nop_getprimarykey_indexname] ('ProductTag') +  '] ON [nopCommerceFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	EXEC(@create_index_text)
END
GO
...
CREATE PROCEDURE [sercan].[FullText_Disable]
AS
BEGIN
	EXEC('
	--drop indexes
...
CREATE PROCEDURE [sercan].[LanguagePackImport]
(
	@LanguageId int,
	@XmlPackage xml,
...
CREATE PROCEDURE [sercan].[DeleteGuests]
(
...
CREATE PROCEDURE [sercan].[CategoryLoadAllPaged]
(
    @ShowHidden         BIT = 0,
...
BEGIN
    ...
    --get category tree
    ;WITH [CategoryTree]
    AS (SELECT [Category].[Id] AS [Id], sercan.[nop_padright] ([Category].[DisplayOrder], '0', @lengthOrder) + '-' + sercan.[nop_padright] ([Category].[Id], '0', @lengthId) AS [Order]
        FROM [Category] WHERE [Category].[ParentCategoryId] = 0
        UNION ALL
        SELECT [Category].[Id] AS [Id], [CategoryTree].[Order] + '|' + sercan.[nop_padright] ([Category].[DisplayOrder], '0', @lengthOrder) + '-' + sercan.[nop_padright] ([Category].[Id], '0', @lengthId) AS [Order]
        FROM [Category]
